...
@Component({
	...
	templateUrl: './aum-mutual-fund.page.html',
	styleUrls: ['./aum-mutual-fund.page.scss'],
})
export class AumMutualFundPage implements OnInit {
							...
							if (res['Body']['GetAAAMFAUMdetailsList'].length === 0) {
								...
								this.aumMutualClientData.forEach((element: any) => {
									this.datas.push({
										...
										total: this.commonService.numberFormatWithCommaUnit(+element['ClientEquityMFAUM'] + +element['ClientDebtMFAUM'])
									})
								});
							}
