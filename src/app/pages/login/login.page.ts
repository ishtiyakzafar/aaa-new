...
export class LoginPage implements OnInit,OnDestroy  {
	...
	public validation_messages = {
		userID: [
			...
			// { type: 'customValidate', message: 'Invalid Char are not allowed' }		review, removed for (custom validation)
	...
	public userTypeTitle:string = "Mobile No. / Partner ID"
	...
	ngOnInit() {
		...
		this.storage.get('isLoggedOut').then((val: any) => {

            if(val == undefined){
                this.storage.set('isLoggedOut', 8);
                setTimeout(() => {
		...
		if(localStorage.getItem('isOneLogin')){
			this.isLoader = true;
			this.serviceFile.getUserDetails(localStorage.getItem('jwtToken'))
			.subscribe((res: any) => {
				if (res) {
					let txtToEncrypt = res['email'] + '_' + res['preferred_username'];
					let checkSumStr = this.encryptWithCryptoJS(txtToEncrypt);
					if (checkSumStr) {
						this.serviceFile.verifyOneLoginUser(localStorage.getItem('jwtToken'), checkSumStr)
	...
	/**
	 * To save loggedIn user details in storage.
	 * @param res 
	 */
	saveToLocal(res: any) {
		...
		localStorage.setItem('userChannel', res['Body']['UserChannel']);
		localStorage.setItem('jwt_token', res['Body']['JwtToken']);
		localStorage.setItem('loginTime', JSON.stringify(new Date()));
		this.getAppID(res['Body']['UserId']);
		this.navigateToDashboard(res);
		this.authService.loggedInStatus.next(true); //To manage auth state
	}

	getClientSettings(): UserManagerSettings {
		return {
		  authority:  environment['oneLoginUrl'],
		  client_id: environment['oneLoginKey'],
		  redirect_uri: environment['oneUrl'],
		  post_logout_redirect_uri: environment['oneUrl'],
	...
	completeAuthentication(): Promise<void> {
	  ...
	  employeeLoginClick(){	
		localStorage.setItem('isOneLogin','true')
		this.startAuthentication();
		this.completeAuthentication().then(() => {
			// if (this.authService.isLoggedIn()) {
			//   this.router.navigate(['protected']);
			// window.open("https://aaauat.indiainfoline.com/", "_blank");
			// }			
		  });
		// this.manager.getUser().then(user => {
		// 	this.user = user;
	...
	proceedwithMultipleClient(){
		...
		this.storage.get('Infltoken').then(Infltoken => {
			...
			//window.location.href ="http://localhost:4200/influencer/dashboard?client_id="+this.selectClientCodeValue+'&token='+Infltoken;
		})
	}
	...
	checkValidation(event?: any, inputValue?: any, multipleIDsCheck?: any) {
		...
		if (alphaNumericRegx.test(value)) {
			...
			if ((numericRegx.test(value) && !alphabet.test(value) && (value.length == 10 || value.length == 7)) && !multipleIDsCheck) {
				this.UserIdFieldValidation = true;
				this.loginType = 2
				// this.validateForm.controls.alphanumericValidation.setValue(true);
			}
	...
	checkValidUser(event?: any, isMobileScreen?: any) {
		this.commonService.setClevertapEvent('OTP_Verify_Clicked', { 'Login ID': this.validateForm.controls['userID']['value'] });
			...
			const id = localStorage.getItem('userId1');
					...
					this.storage.get('appID').then((appId) => {
						if (localStorage.getItem('forgetPswFlag') == 'false' || localStorage.getItem('forgetPswFlag') == null) {
							this.subscription.add(
								this.serviceFile
									.getValidUser(id, appId, localStorage.getItem('loginTypeVal'))
									.subscribe((res: any) => {
										if(res["Head"]["ErrorCode"] == 1 && res["Head"]["ErrorDescription"] == "Invalid User!" && this.checkValidationForMobile(id)){
											this.loader = false;
											localStorage.setItem('forgetPswFlag', 'false');
											localStorage.setItem("GuestMobileNumber", id ? id: "");
											this.toast.displayToast("You have been re-directed to Guest Flow");
											this.guestLogin();
											return;
										}
										this.loader = false;
							...
							this.subscription.add(
								this.serviceFile
									.forgetPin(localStorage.getItem('loginTypeVal'), appId)
									.subscribe((res: any) => {
										this.loader = false;
										...
										if (res['Head']['ErrorCode'] == 0) {
											localStorage.setItem("userId1", res['Body']['UserId']);
											localStorage.setItem("isFirstLogin", 'first');
											this.storage.set('userID', this.ciphetText.getDecryptedValue(res['Body']['UserId']).replace(/[^\d\w]/gi, ''));
											localStorage.setItem('userName', this.ciphetText.getDecryptedValue(res['Body']['UserName']).replace(/[^a-zA-Z ]/g, ""));
										} else {
	...
	next(clicked?: any) {
		...
		if (clicked) {
			...
			setTimeout(() => {
				let idVariable = this.validateForm.controls['userID']['value'] ? this.validateForm.controls['userID']['value'] : id;
						...
						if (localStorage.getItem('forgetPswFlag') == 'false' || localStorage.getItem('forgetPswFlag') == null) {
							this.subscription.add(
								this.serviceFile
									...
									.subscribe((res: any) => {
										if(res["Head"]["ErrorCode"] == 1 && res["Head"]["ErrorDescription"] == "Invalid User!" && this.checkValidationForMobile(idVariable)){
											this.loader = false;
											localStorage.setItem('forgetPswFlag', 'false');
											localStorage.setItem("GuestMobileNumber", idVariable);
											this.toast.displayToast("You have been re-directed to AAA Guest Flow");
											this.guestLogin();
											return;
										}
										this.loader = false;
							...
							this.subscription.add(
								this.serviceFile
									...
									.subscribe((res: any) => {
										...
										localStorage.setItem('forgetPswFlag', 'true');
										if (res['Head']['ErrorCode'] == 0) {
											localStorage.setItem("userId1", this.ciphetText.getDecryptedValue(res['Body']['UserId']).replace(/[^\d\w]/gi, ''));
											localStorage.setItem("isFirstLogin", 'first');
											this.storage.set('userID', this.ciphetText.getDecryptedValue(res['Body']['UserId']).replace(/[^\d\w]/gi, ''));
											localStorage.setItem('userName', this.ciphetText.getDecryptedValue(res['Body']['UserName']).replace(/[^a-zA-Z ]/g, ""));
										} else {
	...
	// on opt submit 
	submitOtp() {
		this.commonService.setClevertapEvent('OTP_Submit', { 'Login ID': localStorage.getItem('userId1') });
		this.loader = true;
		//this.resendTimer = 0;
		this.subscribeOtp?.unsubscribe();
		this.resendTimer = null;
				...
				this.subscription.add(
					this.serviceFile
						...
						.subscribe((res: any) => {
							...
							if(res['Head']['ErrorCode'] == 0){
								let stringValue = localStorage.getItem('userId1') + '_' + moment().format('DDMMYYYY') + '_' + res['Body']['UserPinSet'] + '_' + res['Body']['UserPasswordSet'] + '_' + res['Head']['ErrorCode'];
								this.checkSumValue = this.ciphetText.verifyOtpChecksum(stringValue);
							}
							else if(res['Head']['ErrorCode'] == 1){
								let stringValue = localStorage.getItem('userId1') + '_' + moment().format('DDMMYYYY') + '_' + res['Head']['ErrorCode'];
								this.checkSumValue = this.ciphetText.verifyOtpChecksum(stringValue);
							}
							this.loader = false;
							if(this.checkSumValue == res['Body']['checksum']){
								this.getVerifyOtp(res);
							}
							else{
								this.toast.displayToast('Invalid OTP, Please Try Again!');
							}
			...
			buttons: [
				{
					...
					handler: () => {
						...
						if(this.clip_infl){
							...
							this.userTypeTitle = "Mobile No. / Partner ID"
	...
	public navigateToDashboard(res: any) {
		...
		this.storage.set('isFanChild', 'false');
		if(res && res['Body'] && res['Body']['UserType'] == 'FAN' && res['Body']['UserId'] && res['Body']['UserId'].length > 5){
			this.storage.set('isFanChild', 'true');
		}
		...
		this.storage.set('cookieValue', res['Body']['Cookie']);
		...
		setTimeout(() => {
			...
			localStorage.setItem('brokerageToken', this.bckOfficeCookie);
				...
				this.serviceFile
					...
					.subscribe((res: any) => {
						...
						if (res['Head']['ErrorCode'] == 0) {
							...
							// }, 500);
							//this.toast.displayToast(res['Body']['DisplayMsg'])   
							this.commonService.setClevertapEvent('MPIN_setup', { 'Login ID': localStorage.getItem('userId1') }); 
						}
						else {
							this.toast.displayToast(res['Head']['ErrorDescription'])
						}
	...
	ionViewWillEnter() {
		...
		this.storage.get('token').then((token) => {
			if (token) {
				...
				this.displayLoginCredential = true;
				/* this.storage.get('deviceLock').then( isLock => {
					if (isLock) {
						this.fingerPrintAccess();
					} else {
					...
					if (response['Body']['Status'] == 'SUCCESS' && response['Body']['ClientType'] !== 'INVALID CLIENT') {
						...
						if (reverse) {
							if (this.userType === 'RM' || this.userType === 'FAN') {
								...
								localStorage.setItem('userChannel', this.userChannel);	
			...
			this.serviceFile
				...
				.subscribe((response) => {
					...
					if (+response['body']['status'] === 0 && +response['head']['status'] === 0) {
						...
						localStorage.setItem('JwtToken', response['body']['formAuthToken']);
					} else {
	...
	public redirectTo(redirectFor: any) {
		if (redirectFor === 'register') {
			this.commonService.setClevertapEvent('fan_onboarding_clicked');
	...
	influncerLogin(){
		...
		//window.location.href = "http://localhost:4200/influencer/dashboard?client_id=80302972&token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..WIPGkkni9luuY70w2OCLMb4EODw7Ln8Xb1mW3HNrpQM&mn=7737281411"	
	...
	inflSendOtp(){
		...
		// 		}
		// 	});
		// 	this.otpMsgDisplay = "OTP sent successfully to ******7053"
		// }
		// else{
	...
	/**
	 * to encrypt string using AES ECB mode and convert into Base64 string
	 */
	encryptWithCryptoJS(plainText: string): string {
		const key = CryptoJS.enc.Utf8.parse('AaAz4PpYScdEci0K');
		const iv1 = CryptoJS.enc.Utf8.parse('AaAz4PpYScdEci0K');
	...
	getAppID(cid: any) {
		...
		if (alphaNumericRegx.test(value)) {
			...
			if ((numericRegx.test(value) && !alphabet.test(value) && (value.length == 10 || value.length == 7))) {
				this.loginType = 2
			}
		} else {
			if (value.includes('@')) {
	...
	checkValidationForMobile(inputValue: any) {
		const numericRegx = /^(\+\d{1,3}[- ]?)?\d{10}$/;
		return numericRegx.test(inputValue);
	}
