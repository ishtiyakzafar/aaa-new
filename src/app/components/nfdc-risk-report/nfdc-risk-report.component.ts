...
@Component({
	selector: 'app-nfdc-risk-report',
	providers: [WireRequestService, CustomEncryption],
	templateUrl: './nfdc-risk-report.component.html',
	styleUrls: ['./nfdc-risk-report.component.scss'],
})
export class NfdcRiskReportComponent implements OnInit, OnChanges {
	...
	constructor(private platform: Platform,private toast:ToasterService, private storage: StorageServiceAAA,private route: ActivatedRoute, private wireReqService: WireRequestService, private orderPipe: OrderPipe, private commonService: CommonService, private ciphetText: CustomEncryption) {
	...
	// 		this.wireReqService.getNfdcReport(token, nfdcObj, userID).subscribe((res) => {
	// 			setTimeout(() => {
	// 				this.dataLoad = true;
	// 			}, 400);
	// 			if (res['Head']['ErrorCode'] == 0) {
	// 				this.reportData = res['Body']['NFDCDet'];
	// 				res['Body']['NFDCDet'].forEach(element => {
	...
	// 			}, 400);
	// 			if (res['Head']['ErrorCode'] == 0) {
	// 				//this.riskDataList = res['Body']['RAADet'];
	// 				this.reportData = res['Body']['RAADet'];
	// 				res['Body']['RAADet'].forEach(element => {
	// 					this.riskDataList.push({
	// 						Loginid: element.Loginid,
	// 						Branch: element.Branch,
